...
public class JsonTokenizer {
    ...
    public JsonTokenizer(String json, int pos) {
        this.jsop = json;
    ...
    public String readString() {
        return read(JsonBuilder.STRING);
    }
    ...
    private static String getTokenType(int type) {
        return type <= JsonBuilder.ERROR ? TYPE[type] : "'" + (char) type + "'";
    }
    ...
    private void skipRawValue() {
        switch (currentType) {
            case '[': {
                ...
                while (true) {
                    if (matches(']')) {
                        ...
                        level++;
                    } else if (matches(JsonBuilder.END)) {
            ...
            case '{':
                ...
                break;
            case JsonBuilder.NULL:
            case JsonBuilder.NUMBER:
            case JsonBuilder.TRUE:
            case JsonBuilder.FALSE:
            case JsonBuilder.STRING:
                read();
                break;
            default:
                throw getFormatException(jsop, pos, "value");
